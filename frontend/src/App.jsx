...
export default function HumApp() {
  ...
  useEffect(() => {
    ...
    if (initialSearchesUsed === 'true') {
      ...
      // If initial searches are used, cap searchCount at 3
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const welcomeMessage = remainingSearches === 3 
          ? "🎉 Welcome to hüm! You have 3 free searches to discover your favorite songs!"
    ...
    >
      ...
      <style>{`
        ...
        @media (max-width: 767px) {
          ...
          /* Force Spotify icon to show on mobile when bookmark is clicked - override React inline styles */
          .custom-cursor.show-on-mobile svg.spotify-icon-mobile-show {
            opacity: 1 !important;
      ...
      <div className="relative z-10">
        ...
        <button
          onClick={() => {
            setSearchCount(3);
            localStorage.setItem('hum-search-count', '3');
        ...
        >
          ...
          <div ref={mobileMenuRef} className="relative sm:hidden">
            <button
              ...
              aria-label="Menu"
              className={`flex items-center justify-center px-4 py-3 bg-white/5 backdrop-blur-sm border rounded-full hover:bg-white/10 transition-all ${showMobileMenu ? 'border-[#D8B5FE]' : 'border-white/10'}`}
            >
              {userIcon && getIconImage(userIcon) ? (
                <img src={getIconImage(userIcon)} alt={userIcon} className="w-5 h-5 object-contain flex-shrink-0" />
              ) : userIcon ? (
                <span className="text-base leading-none flex-shrink-0">{userIcon}</span>
              ) : (
                <Menu className="w-5 h-5 text-white/80 flex-shrink-0" strokeWidth={1.5} />
              )}
            </button>
            {showMobileMenu && (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity relative">
                            {/* Spotify Logo - appears on hover (when X button appears) - mobile only */}
                            <div className="md:hidden absolute -left-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                              <svg
                                width="20"
                                height="20"
                                viewBox="0 0 24 24"
